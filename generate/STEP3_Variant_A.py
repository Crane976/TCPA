...
# Ablation Study Variant A: w/o Cluster Focus (Random Tutor Strategy)
...
# 🔥 动态输出文件名 (带数据集后缀)
OUTPUT_CSV_NAME = f'variant_A_no_cluster_{config.CURRENT_DATASET}.csv'
...
def calculate_adaptive_quantity():
    """自适应计算生成数量 (与主脚本保持一致)"""
    print(f"\n🔍 [Variant A] 正在分析测试集规模: {config.CURRENT_DATASET} ...")
...
def inject_watermark(df, key, feature_name):
    ...
    if 'Total Length of Fwd Packets' in df_w.columns:
        total_bytes = df_w['Total Length of Fwd Packets'] + df_w.get('Total Length of Bwd Packets', 0)
        df_w['Flow Bytes/s'] = total_bytes / (duration_sec + epsilon)
    if 'Total Fwd Packets' in df_w.columns:
        total_pkts = df_w['Total Fwd Packets'] + df_w.get('Total Backward Packets', 0)
        df_w['Flow Packets/s'] = total_pkts / (duration_sec + epsilon)
...
def main():
    set_seed(2025)
    print("=" * 60)
    print(f"🚀 [消融实验 Variant A] 无聚类聚焦 (Random Tutor)")
    ...
    # ------------------------------------------------------------------
    # ❌ 核心消融点: 移除聚类 (No Clustering)
    # ------------------------------------------------------------------
    print("\n[步骤1.5] 跳过聚类聚焦机制...")
    print("   -> 策略: Random Sampling (随机选取导师)")
    print("   -> 说明: 这模拟了不知道 Bot 多模态分布，盲目模仿任意 Bot 的情况。")
    ...
    # --- 6. 硬约束 (保留! 这是 Variant A, 不移除硬约束) ---
    print("\n[步骤6] 应用物理硬约束 (Ablation: Yes, 硬约束保留)...")
    ...
    for col in df_final.columns:
        if 'Min' in col:
            base = col.replace(' Min', '')
            if f'{base} Max' in df_final.columns:
                df_final[col] = df_final[col].clip(lower=0)
                df_final[f'{base} Max'] = np.maximum(df_final[f'{base} Max'], df_final[col])
    ...
    print(f"\n✅ Variant A (No Cluster) 生成完毕: {OUTPUT_CSV_PATH}")

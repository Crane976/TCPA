# In a new file: models/train_lstm_hunter.py
import pandas as pd
import numpy as np
import os
import sys
...
# ✅ 2. 为新模型指定新的保存路径
LSTM_HUNTER_MODEL_PATH = os.path.join(project_root, 'models', 'lstm_hunter.pt')
...
def main():
    ...
    print("=" * 60)
    print("🚀 开始训练 LSTM Hunter (Focal Loss + 阈值优化)...")
    print("=" * 60)
    print(f"使用设备: {device}")
    ...
    # ✅ 3. 初始化LSTM模型
    model = LSTM_Classifier(feature_dim=FEATURE_DIM).to(device)
    ...
    for epoch in tqdm(range(EPOCHS), desc="Training"):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), LSTM_HUNTER_MODEL_PATH)

    print(f"\n✅ 训练完成，最佳验证损失: {best_val_loss:.6f}")

    # --- 在验证集上寻找最佳决策阈值 ---
    print("\n[步骤2] 正在验证集上寻找最佳决策阈值...")
    final_model = LSTM_Classifier(feature_dim=FEATURE_DIM).to(device)
    final_model.load_state_dict(torch.load(LSTM_HUNTER_MODEL_PATH, map_location=device))
    final_model.eval()
    with torch.no_grad():
    ...
    # --- 使用最佳阈值在测试集上进行最终评估 ---
    print("\n--- 最终'LSTM Hunter'在【留出测试集】上的真实性能报告 ---")
    with torch.no_grad():

# In a new file: models/train_transformer_hunter.py
import pandas as pd
import numpy as np
import os
import sys
...
# ✅ 1. 导入新的Transformer模型和我们之前定义的FocalLoss
...
# ✅ 2. 为新模型指定新的保存路径
TRANSFORMER_HUNTER_MODEL_PATH = os.path.join(project_root, 'models', 'transformer_hunter.pt')
...
RANDOM_SEED = 2025
BEST_PARAMS = {'learning_rate': 0.0003} # Transformer通常需要更小的学习率
...
def main():
    ...
    print("=" * 60)
    print("🚀 开始训练 Transformer Hunter (Focal Loss + 阈值优化)...")
    print("=" * 60)
    print(f"使用设备: {device}")
    ...
    model = Transformer_Classifier(
        ...
        nhead=1, # 由于feature_dim=23是奇数，nhead必须是1
        num_encoder_layers=2, # 层数
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=7, factor=0.5, verbose=False) # Transformer训练可能更不稳定，patience可以稍大
    ...
    for epoch in tqdm(range(EPOCHS), desc="Training"):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), TRANSFORMER_HUNTER_MODEL_PATH)

    print(f"\n✅ 训练完成，最佳验证损失: {best_val_loss:.6f}")

    # --- 在验证集上寻找最佳决策阈值 ---
    print("\n[步骤2] 正在验证集上寻找最佳决策阈值...")
    final_model = Transformer_Classifier(feature_dim=FEATURE_DIM).to(device)
    final_model.load_state_dict(torch.load(TRANSFORMER_HUNTER_MODEL_PATH, map_location=device))
    final_model.eval()
    with torch.no_grad():
    ...
    # --- 使用最佳阈值在测试集上进行最终评估 ---
    print("\n--- 最终'Transformer Hunter'在【留出测试集】上的真实性能报告 ---")
    with torch.no_grad():
